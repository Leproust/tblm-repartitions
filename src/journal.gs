...
function journalInfo(etape, message, valeur) {
  journaliser(etape, message, valeur, "INFO");
}

function journalErreur(etape, message, valeur) {
  journaliser(etape, message, valeur, "ERREUR");
}

function journalDebug(etape, message, valeur) {
...
function debutChrono(nom) {
  ...
  props.setProperty("CHRONO_" + nom, new Date().getTime());
}
